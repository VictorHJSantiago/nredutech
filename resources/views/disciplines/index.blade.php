...
<div class="main-content">
    <header class="header-section">
        <h1>Disciplinas (Componentes Curriculares)</h1>
        <p class="subtitle">Gerencie as disciplinas disponíveis para as escolas.</p>
    </header>

    @if (session('success'))<div class="alert alert-success mb-4" style="max-width: 1100px; margin-left: auto; margin-right: auto;">{{ session('success') }}</div>@endif
    @if (session('error'))<div class="alert alert-danger mb-4" style="max-width: 1100px; margin-left: auto; margin-right: auto;">{{ session('error') }}</div>@endif
                ...
                @forelse ($componentes as $componente)
                    <tr>
                        ...
                        <td class="actions-cell">
                            @php
                                $user = Auth::user();
                                $isCreator = $user->id_usuario == $componente->id_usuario_criador;                                
                                $canManage = $user->tipo_usuario == 'administrador' ||
                                             $user->tipo_usuario == 'diretor' ||
                                             ($user->tipo_usuario == 'professor' && $isCreator);
                            @endphp
                            @if($componente->status == 'pendente' && $canManage)
                                <form action="{{ route('componentes.update', $componente) }}" method="POST" class="d-inline" title="Aprovar Disciplina">
                                    @csrf
                                    @method('PUT') 
                                    <input type="hidden" name="status" value="aprovado">
                                    <input type="hidden" name="nome" value="{{ $componente->nome }}">
                                    <input type="hidden" name="carga_horaria" value="{{ $componente->carga_horaria }}">
                                    <button type="submit" class="btn-approve">✅</button>
                                </form>
                                <form action="{{ route('componentes.update', $componente) }}" method="POST" class="d-inline" title="Rejeitar Disciplina">
                                    @csrf
                                    @method('PUT') 
